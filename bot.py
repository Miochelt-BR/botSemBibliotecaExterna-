...
def processar_resposta(resposta, nome):
    if resposta == '1':
        print(f'{os.linesep}{nome} na minha visão vale muito apena, isso porque Programação  é uma das areas que mais cresce no mundo e possui salários mensais que vão desde R$2100,00 a até mais R$10000,00 no Brasil, além de contar com uma média anual de $85.000 dolares nos EUA.{os.linesep}')
    elif resposta == '2':
...
def start():
    ...
    while True:
        ...
        resposta = input(
            f'O que gostaria de saber hoje?{os.linesep}[1] - Vale a pena aprender programacao ?{os.linesep}[2] - Quanto tempo leva para conseguir um emprego com programação?{os.linesep}[3] - Quando vou saber que estou BOM o suficiente para conseguir um emprego?{os.linesep}[4] - Onde me recomenda estudar programação para conseguir um emprego hoje?{os.linesep}')
